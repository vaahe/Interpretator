        ...
        for (int i = 0; i < variable.size(); ++i) {
            if (variable[i].name == vec[0]) {
                if (variable[i].type == "amboghj") {
                    ...
                    if (vec.size() > 3 && vec[3] == "+") {
                        *(static_cast<std::string*>(variable[i].value)) = stringOperators(vec[2], vec[4]);
                    }
                    else {
                        bool flag = false;
                        for (int j = 0; j < variable.size(); ++j) {
        ...
        for (int i = 0; i < variable.size(); ++i) {
            for (int j = 0; j < variable.size(); ++j) {
                if (variable[i].name == str1 && variable[j].name == str2) {
                    ...
                    int first = *(static_cast<int*>(variable[j].value));
                    int second = stoi(str1);
                    return first - second;
                }
            }
            if (variable[i].name == str1) {
        ...
        for (int i = 0; i < variable.size(); ++i) {
            for (int j = 0; j < variable.size(); ++j) {
                if (variable[i].name == str1 && variable[j].name == str2) {
                    ...
                    double first = *(static_cast<double*>(variable[j].value));
                    double second = stod(str1);
                    return first + second;
                }
            }
            if (variable[i].name == str1) {
                double first = *(static_cast<double*>(variable[i].value));
                double second = stod(str2);
                return first + second;
            }
        }
    }
        ...
        for (int i = 0; i < variable.size(); ++i) {
            for (int j = 0; j < variable.size(); ++j) {
                if (variable[i].name == str1 && variable[j].name == str2) {
                    ...
                    double first = *(static_cast<double*> (variable[j].value));
                    double second = std::stod(str1);
                    return first - second;
                }
            }
            if (variable[i].name == str1) {
                double first = *(static_cast<double*> (variable[i].value));
                double second = std::stod(str2);
                return first - second;
            }
        }
    }
...
std::string stringOperators(std::string str1, std::string str2) {
    for (int i = 0; i < variable.size(); ++i) {
        for (int j = 0; j < variable.size(); ++j) {
            if (variable[i].name == str1 && variable[j].name == str2) {
